...
- **Brokered credential mode for `/v1`** — lets an embedding host
  ...
  New `handlers/jsonapi/broker.go`; documented in [docs/API.md](api.md).
- **JSON API (`/v1`)** — a clean JSON/REST surface served alongside the HTMX UI,
  ...
  UI is untouched. Documented in [docs/API.md](api.md).
...
- **Optional Postgres storage backend** — a new durable key-value seam
  ...
  **Postgres** store for shared / multi-instance deploys, selected via the new
  ...
  share the same store. Documented in [docs/CONFIGURATION.md](configuration.md#storage).
...
> Documented ahead of release and shipped as part of the [1.10.0](changelog.md#1100-2026-06-22)
