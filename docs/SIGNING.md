# inbrix — request authentication & signing

This document specifies every place inbrix authenticates an HTTP request that is
...
## 0. inbrix emits no webhooks

**inbrix has no outbound webhook emitter and no bespoke webhook HMAC scheme.**

There is no webhook registration surface, no event-delivery queue, no per-tenant
signing secret, no `X-Inbrix AI-Signature` header, and no retry/redelivery
machinery. Nothing in `/v1` causes inbrix to POST an event to a caller-supplied
...
| Real-time new-mail notification, background | Web Push / VAPID (`POST /api/push/subscribe`) — signed by the [Web Push](https://datatracker.ietf.org/doc/html/rfc8292) standard, not by inbrix |
...
The rest of this document covers the three authentication mechanisms inbrix
*does* implement:
...
per-request connection descriptor as headers instead of establishing a inbrix
session. The seam is **off unless `INBRIX_BROKER_SECRET` is set** in the inbrix
process's environment.

### 1.1 Wire format

```http
X-Vulos-Broker-Auth: <the exact value of INBRIX_BROKER_SECRET>
...
1. `INBRIX_BROKER_SECRET` is non-empty. Otherwise → closed.
...
host and inbrix. Anyone who observes one brokered request has the secret *and*
the user's mailbox credential. Treat `INBRIX_BROKER_SECRET` as equivalent in
...
hand a request a scratch S3 bucket, which inbrix uses **only** to cache large,
...
failure → the headers are ignored entirely and inbrix keeps its standalone
IMAP-only behaviour.
...
from making inbrix ship credentials and attachment bytes in the clear to an
arbitrary public endpoint.

**Namespacing.** inbrix writes only under `<X-Vulos-Storage-Prefix>/mail/`, so it
...
This is the **only** HMAC signature inbrix produces. Requests to the object
...
inbrix's own `mail/` sub-space, always ending in `/`):
...
always empty for inbrix's GET/PUT/DELETE (producing an empty line), and the fourth
...
- **Timestamp.** `x-amz-date` is the signing time, taken from the system clock in
  ...
  flight. inbrix enforces **no window of its own** — the acceptance window is the
  S3 server's (AWS's is ±15 minutes). A host with a skewed clock will see requests
  rejected by the server, not by inbrix.
...
`host = s3.example.com`, `bucket = inbrix-test`, `region = eu-west-2`,
...
| canonical URI | `/inbrix-test/tenant-a/mail/attachments/INBOX/42/2.1` |
...
```text
GET␊
/inbrix-test/tenant-a/mail/attachments/INBOX/42/2.1␊
␊
host:s3.example.com␊
...
```text
AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/20260728/eu-west-2/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature=1a277eeb8fddd19cc0201046366b38cfbaae3f22a936e2bc77c2b72f7fad8463
```

#### Vector B — `PUT`, body `inbrix`, session token, key needing encoding
...
| canonical URI | `/inbrix-test/tenant-a/mail/attachments/INBOX/42/invoice%20%231.pdf` |
| payload hash | `c54a21d1cfab8a341d973602e7160654c67a494cd0c17ed14062ee9fd042c19e` |
...
```text
PUT␊
/inbrix-test/tenant-a/mail/attachments/INBOX/42/invoice%20%231.pdf␊
␊
host:s3.example.com␊
x-amz-content-sha256:c54a21d1cfab8a341d973602e7160654c67a494cd0c17ed14062ee9fd042c19e␊
x-amz-date:20260728T123456Z␊
x-amz-security-token:FQoGZXIvYXdzEXAMPLETOKEN␊
␊
host;x-amz-content-sha256;x-amz-date;x-amz-security-token␊
c54a21d1cfab8a341d973602e7160654c67a494cd0c17ed14062ee9fd042c19e
```

Authorization:

```text
AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/20260728/eu-west-2/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date;x-amz-security-token, Signature=49ce5859384d80d60e659ead8404d8b0f8ee424f028143e16031405aee42c308
```
...
HMAC designs. inbrix is **not** one of them — §0. Should inbrix ever need to
emit signed events, the agreed direction is a **shared written spec plus shared
vectors**, not a shared library: nothing inbrix ships may become load-bearing on
another repo staying reachable. This document is inbrix's half of that
