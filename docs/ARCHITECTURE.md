...
inbrix is a Go web application using the [Fiber](https://gofiber.io/) HTTP
...
OAuth2: inbrix runs the full authorization-code flow with PKCE. After callback,
...
`crypto/subtle`). This is the **same auth gate** the mail credential-injection seam uses (`INBRIX_BROKER_SECRET` + `X-Vulos-Broker-Auth`), not a bare on/off
...
standalone inbrix never trusts injected storage headers — the same fail-closed
...
- **Injected-credential mode** (`broker.go`): a first middleware validates
  `X-Vulos-Broker-Auth` against `INBRIX_BROKER_SECRET` (constant-time) and, when
  it matches, parses the `X-Vulos-Mail-*` headers into a per-request connection
  spec so inbrix builds the IMAP/SMTP/DAV client directly from them instead of a
  session. It only ever describes the user's own account; inbrix hosts no mail and
  depends on no central server. Fail-closed: an unset/mismatched secret makes the
  headers ignored entirely, so standalone inbrix never trusts client-supplied
...
loaded at startup. No mail content is persisted by inbrix; it is forwarded and
discarded.
...
[latest release](https://github.com/vul-os/inbrix/releases/latest).
