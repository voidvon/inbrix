...
Inbrix AI is an **independent, self-hostable PIM client** — mail, calendar, and
...
- **Injected-credential ("brokered") mode**, off by default — an embedding
  ...
  Standalone inbrix never trusts client-supplied connection headers unless
...
- `[server] frame_ancestors` config injects into `Content-Security-Policy:
  frame-ancestors` so the Vulos shell can embed Inbrix AI as an iframe
- The `/v1` API is the standalone contract; inbrix hosts no mail and depends
...
- `site/` — a self-contained static landing page + markdown docs viewer (no
  ...
  a bundle you host wherever you like. `site/docs/*.md` are generated by
  `site/gen`, with every link rewritten to resolve against what the bundle
  actually serves — edit the source document and run `make site-docs`
...
- Hosting mail, or growing an account system of its own. inbrix connects to the
