...
```bash
curl -fsSLO https://raw.githubusercontent.com/voidvon/inbrix/v1.17.10/scripts/verify.sh
bash verify.sh --repo voidvon/inbrix --tag v1.17.10 inbrix_1.17.10_linux_amd64.zip
```
...
版本标签，运行 `make check`，并在本机构建 Linux 和 macOS 发布包。全部压缩包
通过 `SHA256SUMS` 校验后，命令会原子推送 `main` 和标签，再使用 `gh` 直接上传
并公开草稿 GitHub Release。补丁版本从 0 递增到 20，之后次版本进一位；
`X.20.0` 的下一版直接为 `(X+1).0.0`。
