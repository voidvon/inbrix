...
try {
  ...
  const flatPdf = await download('下载 PDF（图片）');
  assert(flatPdf.name.endsWith('.pdf'));
  assert(flatPdf.bytes.subarray(0, 5).equals(Buffer.from('%PDF-')));
  const jpg = await download('下载图片');
  assert(jpg.name.endsWith('.jpg'));
  assert.deepEqual([...jpg.bytes.subarray(0, 3)], [0xff, 0xd8, 0xff]);
  ...
  const archive = await download('下载图片');
  ...
  for (const name of names) {
    ...
    assert.deepEqual([...Buffer.from(entries[name]).subarray(0, 3)], [0xff, 0xd8, 0xff]);
  }
  await editor.getByRole('button', { name: '印章管理', exact: true }).click();
  ...
  console.log(`PASS: upload, rename, persistence, quotation/contract insertion, saved stamp retention, JPG/PDF and ${names.length}-page ZIP/PDF, deletion, saved template switching, storage failure handling.`);
